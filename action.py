...
from calibre_plugins.hardcover_sync.common_icons import set_plugin_icon_resources, get_icon
from calibre_plugins.hardcover_sync.common_menus import unregister_menu_actions, create_menu_action_unique
from calibre_plugins.hardcover_sync.common_dialogs import ProgressBarDialog
from calibre_plugins.hardcover_sync.core import (CalibreSearcher, HttpHelper, IdCaches,
                                                 update_calibre_isbn_if_required)
from calibre_plugins.hardcover_sync.dialogs import (DoAddRemoveDialog, DoShelfSyncDialog, SwitchEditionDialog,
                                            PickhardcoverBookDialog, ActionStatus, ChooseShelvesToSyncDialog,
                                            UpdateReadingProgressDialog)

PLUGIN_ICONS = ['images/sync.png',                  'images/refresh.png',
...
class hardcoverSyncAction(InterfaceAction):
    ...
    def rebuild_menus(self):
        ...
        if len(self.users) > 0:
            ...
            # Create menus for linking to hardcover and working with linked books
            create_menu_action_unique(self, m, _('Link to hardcover')+'...', 'images/link_add.png',
                    _('Add, replace or clear link to a hardcover book'),
                    triggered=self.search_hardcover_to_link_book)
            ...
            create_menu_action_unique(self, self.linked_book_submenu, _('View linked book'),
                    'images/view_book.png', _('Open a web browser page showing the linked hardcover book'),
                    triggered=self.view_linked_books)
            create_menu_action_unique(self, self.linked_book_submenu, _('Switch hardcover Edition')+'...',
                    'images/link_add.png', _('Link to a different edition of a hardcover book'),
            ...
            create_menu_action_unique(self, self.linked_book_submenu, _('Remove link'),
                    'images/link_delete.png', _('Clear the link with hardcover'),
    ...
    def are_selected_books_linked(self):
        ...
        for row in rows:
            ...
            if 'hardcover' not in identifiers:
                all_has_hardcover_id = False
                break
        return all_has_hardcover_id
    ...
    def add_or_remove_to_shelf(self, action, user_name):
        ...
        # Ensure our hardcover id mapping caches are reset
        self.id_caches.invalidate_caches()
        ...
        if d.result() == d.Accepted:
            ...
            self._update_hardcover_ids(calibre_books, msg, previous)
    ...
    def sync_shelves(self, user_name):
        ...
        if choose_dialog.hardcover_shelf_books is None:
            return error_dialog(self.gui, _('Unable to Sync'),
        ...
        d = DoShelfSyncDialog(self.gui, self, self.grhttp, user_name, choose_dialog.selected_shelves,
                              choose_dialog.hardcover_shelf_books, self.calibre_searcher)
        d.exec_()
        if d.result() == d.Accepted:
            ...
            self.progressbar_show(len(d.hardcover_books))
            # When finally exiting, update the hardcover Id and ISBN where any were changed
            self._update_calibre_database_ids_after_sync(d.hardcover_books)
            ...
            updated_ids = [book['calibre_id'] for book in d.hardcover_books if book['updated']]
    ...
    def _update_hardcover_ids(self, calibre_books, msg, previous):
        # When finally exiting, update the hardcover Id and ISBN where any were changed
    ...
    def _update_calibre_database_ids_after_sync(self, hardcover_books):
        # Our collection of books are hardcover books on the shelf
        db = self.gui.library_view.model().db
        gr_cache = self.id_caches.hardcover_to_calibre_ids()
        cb_cache = self.id_caches.calibre_to_hardcover_ids()
        for book in hardcover_books:
            ...
            book['updated'] = True
            if calibre_id:
                # A Calibre book has been linked to a hardcover one
                db.set_identifier(calibre_id, 'hardcover', hardcover_id, commit=False)
                ...
                # a different hardcover id for a Calibre book.
                db.set_identifier(orig_calibre_id, 'hardcover', '', commit=False)
                # Be careful when updating caches as may have already overwritten data
                if cb_cache[orig_calibre_id] == hardcover_id:
                    del cb_cache[orig_calibre_id]
                # We need to maintain our in-memory cache of mapped ids.
                calibre_ids_mapped = gr_cache.get(hardcover_id, [])
                calibre_ids_mapped.remove(orig_calibre_id)
                if len(calibre_ids_mapped) == 0:
                    del gr_cache[hardcover_id]
                else:
                    gr_cache[hardcover_id] = calibre_ids_mapped

        db.commit()

    def search_hardcover_to_link_book(self):
        if not self._is_valid_selection():
            return
        previous = self.gui.library_view.currentIndex()
        calibre_books = self._convert_selection_to_books()
    ...
    def search_to_link_books(self, calibre_books):
        ...
        for row, calibre_book in enumerate(calibre_books):
            ...
            hardcover_id_for_isbn = ''
            hardcover_isbn = None
            hardcover_books = []
            if isbn:
                # As we have a Calibre ISBN, lets attempt a lookup by that first
                hardcover_id_for_isbn = self.grhttp.get_hardcover_id_for_isbn(isbn)
                if hardcover_id_for_isbn:
                    hardcover_book = self.grhttp.get_hardcover_book_for_id(hardcover_id_for_isbn)
                    if hardcover_book is None:
                        # Have a really unusual situation!
                        # Lets pretend we don't have a hardcover id after all
                        hardcover_id_for_isbn = None
                    else:
                        hardcover_books = [hardcover_book]
                        hardcover_isbn = hardcover_book['hardcover_isbn']
            ...
            is_isbn_match = hardcover_id_for_isbn is not None and len(hardcover_id_for_isbn) > 0
            d = PickhardcoverBookDialog(self.gui, self.grhttp, self.id_caches, calibre_book,
                                        hardcover_books, next_book, is_isbn_match)
            d.exec_()
            if d.result() != d.Accepted:
                break
            if d.skip:
            ...
            if calibre_book['orig_hardcover_id'] != hardcover_id:
            ...
            if update_isbn == 'ALWAYS' or missing_isbn:
                ...
                if hardcover_id != hardcover_id_for_isbn:
                    hardcover_isbn = None
                    hardcover_book = self.grhttp.get_hardcover_book_for_id(hardcover_id)
                    if hardcover_book:
                        hardcover_isbn = hardcover_book['hardcover_isbn']
                if hardcover_isbn is not None:
                    update_calibre_isbn_if_required(calibre_book, hardcover_isbn, update_isbn)
    ...
    def _update_calibre_database_ids_for_selection(self, calibre_books):
        ...
        db = self.gui.library_view.model().db
        _gr_cache = self.id_caches.hardcover_to_calibre_ids()
        _cb_cache = self.id_caches.calibre_to_hardcover_ids()
        for book in calibre_books:
            ...
            calibre_id = book['calibre_id']
            hardcover_id = book['hardcover_id']
            orig_hardcover_id = book.get('orig_hardcover_id', None)
            isbn = book['calibre_isbn']
            orig_isbn = book['orig_calibre_isbn']
            if isbn and isbn != orig_isbn:
                db.set_isbn(calibre_id, isbn, notify=False, commit=False)
                book['updated'] = True
            if hardcover_id == orig_hardcover_id:
                continue
            db.set_identifier(calibre_id, 'hardcover', hardcover_id, notify=False, commit=False)
            book['updated'] = True
        db.commit()
    ...
    def _convert_selection_to_books(self):
        ...
        for row in rows:
            ...
            book['orig_hardcover_id'] = book['hardcover_id']
    ...
    def remove_links(self):
        ...
        if not question_dialog(self.gui, _('Are you sure?'), '<p>' +
                               _("Removing any linked hardcover ids may result in you having to manually " 
        ...
        try:
            ...
            cb_cache = self.id_caches.calibre_to_hardcover_ids()
            updated_ids = []
            self.gui.status_bar.showMessage(_('Removing hardcover ids from books')+'...')
            for row in rows:
                calibre_id = db.id(row.row())
                db.set_identifier(calibre_id, 'hardcover', '', commit=False)
                hardcover_id = cb_cache[calibre_id]
    ...
    def view_linked_books(self):
        ...
        for row in rows:
            calibre_id = db.id(row.row())
            identifiers = db.get_identifiers(calibre_id, index_is_id=True)
            hardcover_id = identifiers.get('hardcover', None)
            if hardcover_id:
                self.grhttp.view_book_on_hardcover(hardcover_id)
    ...
    def switch_linked_edition(self):
        ...
        for row in rows:
            ...
            hardcover_id = identifiers.get('hardcover', None)
            if not hardcover_id:
                error_dialog(self.gui, _('No hardcover Id'),
                   _('You cannot switch editions for a book that is not already linked to hardcover.<br>'),
                   show=True)
                return
        calibre_books = self._convert_selection_to_books()
        if not calibre_books:
    ...
    def switch_edition_for_linked_books(self, calibre_books):
        ...
        for row, calibre_book in enumerate(calibre_books):
            ...
            identifiers = self.db.get_identifiers(calibre_id, index_is_id=True)
            hardcover_id = identifiers.get('hardcover', None)
            # Lookup the book to get the hardcover work id to query for editions
            hardcover_book = self.grhttp.get_hardcover_book_with_work_id(hardcover_id)
            if hardcover_book is None:
                # Eeek!
                continue
            work_id = hardcover_book['hardcover_work_id']
            if work_id is None:
                # Eeek!
                continue
            # Now we need to lookup the edition information for this work id.
            # There is an API call for this, but for some reason hardcover have it permissioned
    ...
    def download_tags(self, user_name, is_download):
        ...
        for row in rows:
            calibre_id = db.id(row.row())
            hardcover_id = self.id_caches.calibre_to_hardcover_ids().get(calibre_id, None)
            if not hardcover_id:
                continue
            book = self.grhttp.get_review_book(user_name, hardcover_id)
            if not book:
                continue
            ...
            for shelf_name in book['hardcover_shelves'].split(', '):
    ...
    def upload_tags(self, user_name, is_download):
        ...
        for row in rows:
            ...
            book = self.grhttp.get_review_book(user_name, hardcover_id)
            if book:
                current_shelves = book['hardcover_shelves'].split(', ')
            else:
                current_shelves = []
            ...
            for calibre_tag in calibre_tags:
                for shelf_name, mapped_tag_values in tag_mappings.items():
                    ...
                    self.grhttp.add_remove_book_to_shelf(oauth_client, shelf_name, hardcover_id)
                    shelve_additions += 1
    ...
    def progressbar(self, window_title=_("hardcover Sync progress"), on_top=False, show=False):
