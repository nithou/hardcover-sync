...
from calibre_plugins.hardcover_sync.common_compatibility import qSizePolicy_Expanding, qSizePolicy_Minimum
from calibre_plugins.hardcover_sync.common_icons import get_icon
from calibre_plugins.hardcover_sync.common_dialogs import SizePersistedDialog, KeyboardConfigDialog
from calibre_plugins.hardcover_sync.common_widgets import (NoWheelComboBox, ReadOnlyTextIconWidgetItem,
                                                    CheckableTableWidgetItem, ReadOnlyTableWidgetItem,
                                                    ImageTitleLayout, CustomColumnComboBox)

HELP_URL = 'https://github.com/kiwidude68/calibre_plugins/wiki/hardcover-Sync'
...
SYNC_CUSTOM_COLUMN_DEFAULT_LOOKUP_TAGS               = '#hardcover_tags'
SYNC_CUSTOM_COLUMN_DEFAULT_LOOKUP_RATING             = '#hardcover_rating'
SYNC_CUSTOM_COLUMN_DEFAULT_LOOKUP_DATE_READ          = '#hardcover_date_read'
SYNC_CUSTOM_COLUMN_DEFAULT_LOOKUP_REVIEW_TEXT        = '#hardcover_review'
SYNC_CUSTOM_COLUMN_DEFAULT_LOOKUP_READING_PROGRESS   = '#hardcover_reading_progress'
CUSTOM_COLUMN_DEFAULTS = {
                SYNC_CUSTOM_COLUMN_DEFAULT_LOOKUP_TAGS : {
                    'column_heading': _("hardcover Tags"),
                    'datatype' : 'text',
                    'is_multiple' : True,
                    'description' : _("hardcover tags used for mapping to calbre."),
                ...
                SYNC_CUSTOM_COLUMN_DEFAULT_LOOKUP_REVIEW_TEXT : {
                    'column_heading': _("hardcover Review"),
                    'datatype' : 'comments',
                ...
                SYNC_CUSTOM_COLUMN_DEFAULT_LOOKUP_READING_PROGRESS : {
                    'column_heading': _("hardcover Progress"),
                    'datatype' : 'int',
...
URL = 'http://www.hardcover.app'
URL_HTTPS = 'https://www.hardcover.app'

# This is where all preferences for this plugin will be stored
plugin_prefs = JSONConfig('plugins/hardcover Sync')
...
def migrate_config_if_required():
    ...
    if schema_version < 1.6:
        if DEBUG:
            prints('Migrating hardcover Sync schema from prior to 1.6')
        ...
        hardcover_settings = plugin_prefs[STORE_PLUGIN]
        hardcover_settings[KEY_TAG_MAPPING_COLUMN] = tag_mapping_column
        if 'searchLinkShortcut' in hardcover_settings:
            del hardcover_settings['searchLinkShortcut']
        if 'viewLinkShortcut' in hardcover_settings:
            del hardcover_settings['viewLinkShortcut']
        plugin_prefs[STORE_PLUGIN] = hardcover_settings

    # Check for user upgrading from prior to 1.68
    if schema_version < 1.68:
        if DEBUG:
            prints('Migrating hardcover Sync schema from prior to 1.68')
        users_settings = plugin_prefs[STORE_USERS]
        for user_info in users_settings.values():
            for shelf in user_info[KEY_SHELVES]:
        ...
        hardcover_settings = plugin_prefs[STORE_PLUGIN]
        hardcover_settings[KEY_REVIEW_TEXT_COLUMN] = ''
        plugin_prefs[STORE_PLUGIN] = hardcover_settings
...
class DateTimeColumnComboBox(NoWheelComboBox):
    ...
    VALUE_SYNC_MAP = OrderedDict([
                       ('none', _('None')),
                       ('today', _('Today')),
                       ('read_at', _('hardcover Date Read')),
                       ('date_added', _('hardcover Date Added')),
                       ('date_updated', _('hardcover Date Updated')),
                       ('started_at', _('hardcover Date Started'))
                      ])
...
class AddShelfDialog(QDialog):
    ...
    def __init__(self, parent=None, shelves=[]):
        ...
        self.setWindowTitle(_('Create hardcover shelf'))
...
class MaintainActionsDialog(SizePersistedDialog):
    ...
    def __init__(self, parent, is_shelf_add_actions, shelves, custom_columns, all_tags):
        SizePersistedDialog.__init__(self, parent, 'hardcover sync plugin:maintain sync action dialog')
        ...
        if is_shelf_add_actions:
            rating_title = _('Upload rating to hardcover when adding to this shelf')
            date_read_title = _('Upload date read to hardcover when adding to this shelf')
            review_text_title = _('Upload review text to hardcover when adding to this shelf')
            ...
            rating_title = _('Sync rating from hardcover when syncing from this shelf')
            date_read_title = _('Sync date read from hardcover when syncing from this shelf')
            review_text_title = _('Sync review text from hardcover when syncing from this shelf')
...
class ShelvesTableWidget(QTableWidget):
    ...
    def create_context_menu(self):
        ...
        self.addAction(sep2)
        self.edit_shelves_action = QAction(get_icon('images/view_book.png'), _('Edit Shelves on hardcover'), self)
        self.edit_shelves_action.triggered.connect(self.edit_shelves_on_hardcover)
    ...
    def edit_shelves_on_hardcover(self):
...
class ConfigWidget(QWidget):
    ...
    def __init__(self, plugin_action, grhttp):
        ...
        user_label = QLabel(_('Select hardcover user:'), self)
        ...
        self._auth_button = QPushButton(_('Authorize Plugin with hardcover'), self)
        ...
        self._sync_shelves_button.setToolTip(_('Refresh the list of shelves and book counts from hardcover'))
        ...
        isbn_label = QLabel(_('When linking to hardcover:'), self)
    ...
    def add_user_profile(self):
        ...
        new_user_name, ok = QInputDialog.getText(self, _('Add new user'),
                    _('You must create a profile for each hardcover user you will connect to.<br>Enter a unique display name for this user profile:'), text='Default')
        if not ok:
            # Operation cancelled
            return
    ...
    def authorize_plugin(self):
        ...
        if not question_dialog(self, 
                               ...
                               '<p>'+_("Have you clicked 'Allow access' for this plugin on the hardcover website?")):
            return
        ...
        # Now also retrieve the hardcover user id so we can use it in web queries
        user_id = self.grhttp.get_hardcover_user_id(oauth_token=user_token, oauth_secret=user_secret)
        if not user_id:
            return error_dialog(self, _('hardcover failure'), _('Unable to obtain the user id'), show=True)
    ...
    def add_shelf(self):
        ...
        # Perform the update with hardcover
