...
import calibre_plugins.hardcover_sync.config as cfg
from calibre_plugins.hardcover_sync.common_compatibility import qSizePolicy_Minimum, qtDropActionCopyAction, qtDropActionMoveAction
from calibre_plugins.hardcover_sync.common_icons import get_icon, get_pixmap
from calibre_plugins.hardcover_sync.common_dialogs import SizePersistedDialog
from calibre_plugins.hardcover_sync.common_widgets import (DateDelegate, DateTableWidgetItem,
                            ImageTitleLayout, ReadOnlyTableWidgetItem, ReadOnlyLineEdit, get_date_format)
from calibre_plugins.hardcover_sync.core import update_calibre_isbn_if_required, get_searchable_author, CalibreDbHelper
...
def get_urls_from_event(event):
    ...
    and represent urls on the hardcover site.
    '''
...
class SwitchEditionTableWidget(QTableWidget):
    ...
    def create_context_menu(self):
        ...
        self.view_book_action = QAction(_('View &book on hardcover.com'), self)
        self.view_book_action.setIcon(get_icon('images/view_book.png'))
        self.view_book_action.triggered.connect(self.view_book_on_hardcover)
        self.addAction(self.view_book_action)
        sep1 = QAction(self)
        sep1.setSeparator(True)
        self.addAction(sep1)
        self.paste_url_action = QAction(_('Paste hardcover.com url'), self)
    ...
    def populate_table(self, hardcover_edition_books):
        self.clear()
        self.setAlternatingRowColors(True)
        self.setRowCount(len(hardcover_edition_books))
        ...
        self.hardcover_edition_books = hardcover_edition_books
        for row, book in enumerate(hardcover_edition_books):
        ...
        if len(hardcover_edition_books) > 0:
            self.selectRow(0)
    ...
    def populate_table_row(self, row, hardcover_edition_book):
        # Check to see whether we have one or more calibre_ids matching this
        # hardcover id in our cache
        existing_calibre_ids = self.id_caches.get_calibre_ids_linked(
                                        hardcover_edition_book['hardcover_id'])
        title_item = hardcoverTitleWidgetItem(hardcover_edition_book['hardcover_title'],
                                              existing_calibre_ids, self.calibre_id)
        title_item.setData(Qt.UserRole, row)
        self.setItem(row, 0, title_item)
        self.setItem(row, 1, ReadOnlyTableWidgetItem(hardcover_edition_book['hardcover_cover']))
        self.setItem(row, 2, ReadOnlyTableWidgetItem(hardcover_edition_book['hardcover_edition']))
    ...
    def selected_hardcover_book(self):
        row = self.selectionModel().selectedRows()[0]
        return self.hardcover_edition_books[self.item(row.row(), 0).data(Qt.UserRole)]

    def view_book_on_hardcover(self):
        url = '%s/book/show/%s' % (cfg.URL, self.selected_hardcover_book()['hardcover_id'])
        open_url(QUrl(url))
    ...
    def dropEvent(self, event):
        ...
        # User has dropped a valid hardcover url onto our dialog.
    ...
    def add_url_to_grid(self, url):
        ...
        hardcover_edition_book = {}
        hardcover_edition_book['hardcover_id'] = hardcover_id
        hardcover_edition_book['hardcover_title'] = url
        hardcover_edition_book['hardcover_cover'] = ''
        hardcover_edition_book['hardcover_edition'] = ''
        hardcover_edition_book['hardcover_isbn'] = ''
        self.hardcover_edition_books.insert(0, hardcover_edition_book)
        self.populate_table(self.hardcover_edition_books)
        self.selectRow(0)
...
class SwitchEditionDialog(SizePersistedDialog):
    '''
    This dialog allows the user to pick a book from search results from hardcover
    '''
    def __init__(self, parent, id_caches, calibre_book, hardcover_books,
                 next_book, enable_search=True):
        SizePersistedDialog.__init__(self, parent, 'hardcover sync plugin:switch edition dialog')
        self.calibre_book = calibre_book
        self.skip = False
        self.setWindowTitle(_('Switch hardcover Edition'))
        layout = QVBoxLayout(self)
        self.setLayout(layout)
        window_title = _('hardcover.com Editions')
        ...
        layout.addSpacing(5)
        layout.addWidget(QLabel(_('Select the hardcover edition to link to this calibre book:'), self))
        ...
        message = _('You can drag/drop a hardcover website link to add it to the results.')
        ...
        if enable_search:
            search_button = self.button_box.addButton(_('Search hardcover.com'), QDialogButtonBox.ResetRole)
            search_button.clicked.connect(self.search_on_hardcover)
        if next_book:
        ...
        # Populate with data
        self.pick_book_table.populate_table(hardcover_books)
        # Cause our dialog size to be restored from prefs or created on first usage
        self.resize_dialog()

    def search_on_hardcover(self):
    ...
    def selected_hardcover_book(self):
        book = self.pick_book_table.selected_hardcover_book()
        return book
...
class ChooseShelvesToSyncDialog(SizePersistedDialog):
    ...
    def __init__(self, parent=None, plugin_action=None, grhttp=None, user_name=None, shelves=[]):
        SizePersistedDialog.__init__(self, parent, 'hardcover sync plugin:shelves sync dialog')
    ...
    def _accept_clicked(self):
        ...
        self.hardcover_shelf_books = self.grhttp.get_hardcover_books_on_shelves(self.user_name, self.selected_shelves)
...
class UpdateReadingProgressTableWidget(QTableWidget):
    ...
    search_for_hardcover_books = pyqtSignal(object, object)
    ...
    def __init__(self, parent, reading_progress_column, rating_column=None, date_read_column=None, review_text_column=None):
        ...
        self.create_context_menu()
        self.itemSelectionChanged.connect(self.item_selection_changed)
        self.doubleClicked.connect(self.search_for_hardcover_books_click)
    ...
    def create_context_menu(self):
        self.setContextMenuPolicy(Qt.ActionsContextMenu)
        self.search_action = QAction(get_icon('images/link_add.png'), _('Search for book in hardcover')+'...', self)
        self.search_action.triggered.connect(self.search_for_hardcover_books_click)
        self.addAction(self.search_action)
        sep1 = QAction(self)
        sep1.setSeparator(True)
        self.addAction(sep1)
        self.view_book_action = QAction(get_icon('images/view_book.png'), _('&View book on hardcover.com'), self)
        self.view_book_action.triggered.connect(self.view_book_on_hardcover_click)
        self.addAction(self.view_book_action)
    ...
    def item_selection_changed(self):
        selection_has_no_hardcover_id = True
        selection_is_not_valid = True
        if not self.selectionModel().hasSelection():
            selection_has_no_hardcover_id = False
            ...
            for row in self.selectionModel().selectedRows():
                ...
                if calibre_book['status'] == ActionStatus.VALID:
                    selection_is_not_valid = False
                if calibre_book['hardcover_id']:
                    selection_has_no_hardcover_id = False
        self.book_selection_changed.emit(selection_is_not_valid)
        self.search_action.setEnabled(selection_is_not_valid)
        self.view_book_action.setEnabled(not selection_has_no_hardcover_id)

    def view_book_on_hardcover_click(self):
        for row in self.selectionModel().selectedRows():
            calibre_book_id = self.item(row.row(), 2).data(Qt.UserRole)
            calibre_book = self.calibre_books[calibre_book_id]
            self.view_book.emit(calibre_book['hardcover_id'])

    def search_for_hardcover_books_click(self):
        rows = []
        ...
        if len(calibre_books_to_search) == 0:
            return
        self.search_for_hardcover_books.emit(rows, calibre_books_to_search)
...
class UpdateReadingProgressDialog(SizePersistedDialog):
    ...
    def __init__(self, parent, plugin_action, grhttp, id_caches, user_name, action, calibre_books):
        SizePersistedDialog.__init__(self, parent, 'hardcover sync plugin:update reading progress dialog')
        self.gui = parent
    ...
    def init_gui_layout(self):
        ...
        message = _('To fix missing links, double click to search for a matching book in hardcover')
        ...
        self.summary_table.view_book.connect(self.grhttp.view_book_on_hardcover)
        self.summary_table.search_for_hardcover_books.connect(self.handle_search_for_hardcover_books)
        ...
        self.search_button = button_box.addButton( _('Search hardcover')+'...', QDialogButtonBox.ResetRole)
        self.search_button.clicked.connect(self.summary_table.search_for_hardcover_books_click)
    ...
    def update_book_status(self, calibre_book):
        ...
        hardcover_id = calibre_book['hardcover_id']
        isbn = calibre_book['calibre_isbn']
        if not hardcover_id and not isbn:
            calibre_book['status_msg'] = _('No hardcover book linked!')
            calibre_book['status'] = ActionStatus.NO_LINK
            return
        if not hardcover_id and isbn:
            # Now see if we can find it by an API call to hardcover for this ISBN
            hardcover_id = self.grhttp.get_hardcover_id_for_isbn(isbn)
            if hardcover_id:
                calibre_book['hardcover_id'] = hardcover_id
            else:
                calibre_book['status'] = ActionStatus.NO_LINK
                calibre_book['status_msg'] = _('No hardcover book linked!')
                return
    ...
    def find_hardcover_id_on_shelf_contents(self, hardcover_shelf_books, isbn):
        # Currently we will only match on ISBN. Maybe in future will do title/author too
        if not isbn:
            return None
        for hardcover_id, hardcover_book in hardcover_shelf_books.items():
            if hardcover_book['hardcover_isbn'] == isbn:
                return hardcover_id
    ...
    def action_button_clicked(self):
        ...
        read_books = []
        self.plugin_action.progressbar_label(_('Updating hardcover progress')+'...')
        ...
        for calibre_book in self.calibre_books:
            ...
            if calibre_book['status'] == ActionStatus.VALID:
                ...
                self.grhttp.update_status(client, hardcover_id, progress, self.progress_is_percent, review_text)
                if (upload_progress and progress):
                    calibre_book['hardcover_reading_progress'] = progress

                    if self.put_reading_on_currently_reading_shelf_checked and progress < 100:
                        currently_reading_books.append(calibre_book)
                        review_id = self.grhttp.add_remove_book_to_shelf(client, CURRENTLY_READING_SHELF, hardcover_id, 'add')

                    if self.put_finished_on_read_shelf_checked and progress >= 100:
                        read_books.append(calibre_book)
                        review_id = self.grhttp.add_remove_book_to_shelf(client, READ_SHELF, hardcover_id, 'add')
                        # If adding books and rating/date read columns update the hardcover review
                        if review_id:
                            if review_id and (upload_rating or upload_date_read or upload_review_text):
                                ...
                                if upload_rating:
                                    ...
                                    if rating:
                                        calibre_book['hardcover_rating'] = rating
                                if upload_date_read:
                                    date_read = calibre_book['calibre_date_read']
                                    if date_read:
                                        calibre_book['hardcover_read_at'] = date_read
                                if upload_review_text:
                                    review_text = calibre_book['calibre_review_text']
                                    if review_text:
                                        calibre_book['hardcover_review_text'] = review_text
                                self.grhttp.update_review(client, READ_SHELF, review_id, hardcover_id, rating, date_read, review_text)
    ...
    def _apply_add_actions_for_books(self, added_books, shelf_name, upload_progress, upload_rating=None, upload_date_read=None, upload_review_text=None):
        ...
        if upload_progress:
            update_progress_action = {'action':'ADD', 'column':self.reading_progress_column, 'value':'hardcover_reading_progress'}
            add_actions.append(update_progress_action)
            if self.put_finished_on_read_shelf_checked:
                if upload_rating:
                    update_rating_action = {'action':'ADD', 'column':self.rating_column, 'value':'hardcover_rating'}
    ...
    def handle_search_for_hardcover_books(self, rows, calibre_books):
        for index, row in enumerate(rows):
            ...
            hardcover_books = self.grhttp.search_for_hardcover_books(title, author)
            d = PickhardcoverBookDialog(self, self.grhttp, self.id_caches, calibre_book,
                                        hardcover_books, next_book)
            d.exec_()
            if d.skip:
                continue
            if d.result() != d.Accepted:
                return
            hardcover_book = d.selected_hardcover_book()
            if hardcover_book is None:
                continue
            hardcover_id = hardcover_book['hardcover_id']
            calibre_book['hardcover_id'] = hardcover_id
            ...
            if self.update_isbn == 'ALWAYS' or missing_isbn:
                ...
                # Necessary because ISBN is not returned by the hardcover search API
                hardcover_book = self.grhttp.get_hardcover_book_for_id(hardcover_id)
                if hardcover_book:
                    update_calibre_isbn_if_required(calibre_book, hardcover_book['hardcover_isbn'],
                                                    self.update_isbn)
...
class CalibreTitleWidgetItem(SortableReadOnlyTableWidgetItem):
    ...
    def __init__(self, title, existing_hardcover_id, match_hardcover_id, sort_key=None):
        super(CalibreTitleWidgetItem, self).__init__(title, sort_key)
        if existing_hardcover_id:
            if match_hardcover_id == existing_hardcover_id:
                ...
                self.setToolTip(_('This calibre book is linked to a different hardcover book.'))


class hardcoverTitleWidgetItem(ReadOnlyTableWidgetItem):
    ...
    def __init__(self, title, existing_calibre_ids, match_calibre_id):
        ...
        if existing_calibre_ids and len(existing_calibre_ids) > 0:
            if match_calibre_id in (existing_calibre_ids):
                self.setIcon(get_icon('metadata.png'))
                self.setToolTip(_('You have already linked to this hardcover book'))
            else:
                self.setIcon(get_icon('dialog_warning.png'))
                self.setToolTip(_('This hardcover book is linked to a different calibre book.'))
...
class StatusDataWidgetItem(ReadOnlyTableWidgetItem):
    ...
    TOOLTIP_MAP = { ActionStatus.NO_LINK: _('You must link this calibre book to a matching book in hardcover'),
                    ActionStatus.WARNING: _('No changes will be made to your shelf in hardcover for this book'),
                    ActionStatus.VALID: '' }
...
class SyncStatusDataWidgetItem(StatusDataWidgetItem):
    ...
    TOOLTIP_MAP = { ActionStatus.NO_LINK: _("You must link this hardcover book to a matching book in calibre \n"
                    ...
                    ActionStatus.ADD_EMPTY: _("An empty book will be created in calibre for this book \n"
                                            "using the hardcover metadata") }


class PickhardcoverBookTableWidget(QTableWidget):
    ...
    def populate_table(self, hardcover_search_books):
        self.clear()
        self.setAlternatingRowColors(True)
        self.setRowCount(len(hardcover_search_books))
        ...
        self.hardcover_search_books = hardcover_search_books
        for row, book in enumerate(hardcover_search_books):
            self.populate_table_row(row, book)
        ...
        self.setSortingEnabled(True)
        self.setMinimumSize(500, 0)
        if len(hardcover_search_books) > 0:
            self.selectRow(0)
    ...
    def populate_table_row(self, row, hardcover_search_book):
        # Check to see whether we have one or more calibre_ids matching this
        # hardcover id in our cache
        existing_calibre_ids = self.id_caches.get_calibre_ids_linked(
                                        hardcover_search_book['hardcover_id'])
        title_item = hardcoverTitleWidgetItem(hardcover_search_book['hardcover_title'],
                                              existing_calibre_ids, self.calibre_id)
        title_item.setData(Qt.UserRole, row)
        self.setItem(row, 0, title_item)
        self.setItem(row, 1, ReadOnlyTableWidgetItem(hardcover_search_book['hardcover_author']))
        self.setItem(row, 2, ReadOnlyTableWidgetItem(hardcover_search_book['hardcover_series']))
    ...
    def selected_hardcover_book(self):
        ...
        if row >= 0:
            return self.hardcover_search_books[row]
    ...
    def add_url_to_grid(self, url):
        ...
        if not match:
            return
        hardcover_id = match.group(1)
        hardcover_search_book = {}
        hardcover_search_book['hardcover_id'] = hardcover_id
        hardcover_search_book['hardcover_title'] = url
        hardcover_search_book['hardcover_author'] = ''
        hardcover_search_book['hardcover_series'] = ''
        self.hardcover_search_books.insert(0, hardcover_search_book)
        self.populate_table(self.hardcover_search_books)
        self.selectRow(0)
...
class PickhardcoverBookDialog(SizePersistedDialog):
    '''
    This dialog allows the user to pick a book from search results from hardcover
    '''
    def __init__(self, parent, grhttp, id_caches, calibre_book, hardcover_books,
                 next_book, is_isbn_match=False):
        SizePersistedDialog.__init__(self, parent, 'hardcover sync plugin:pick hardcover book dialog')
        self.calibre_book = calibre_book
        self.grhttp, self.id_caches = grhttp, id_caches
        self.skip = False
        self.setWindowTitle(_('Search for hardcover book'))
        layout = QVBoxLayout(self)
        self.setLayout(layout)
        window_title = _('hardcover.com matches for Title/Author')
        if is_isbn_match:
            window_title = _('hardcover.com matches for ISBN')
        title_layout = ImageTitleLayout(self, 'images/link_add_lg.png', window_title)
        layout.addLayout(title_layout)
        ...
        layout.addSpacing(5)
        layout.addWidget(QLabel(_('Select the hardcover book to link to this calibre book:'), self))
        self.pick_book_table = PickhardcoverBookTableWidget(self, id_caches,
                                                            calibre_book['calibre_id'])
        layout.addWidget(self.pick_book_table)
        self.pick_book_table.doubleClicked.connect(self.accept)
        self.pick_book_table.book_selection_changed.connect(self.handle_book_selection_changed)
        ...
        # Populate with data
        self.pick_book_table.populate_table(hardcover_books)
        self.handle_book_selection_changed(len(hardcover_books) > 0)
        # Cause our dialog size to be restored from prefs or created on first usage
        self.resize_dialog()
    ...
    def selected_hardcover_book(self):
        return self.pick_book_table.selected_hardcover_book()
    ...
    def switch_editions(self):
        ...
        hardcover_book = self.pick_book_table.selected_hardcover_book()
        if not hardcover_book:
            error_dialog(self, _('No book selected'), _('You must select a book to switch edition'), show=True)
            return
        work_id = hardcover_book.get('hardcover_work_id', None)
        if not work_id:
            # Determine
            work_hardcover_book = self.grhttp.get_hardcover_book_with_work_id(hardcover_book['hardcover_id'])
            if not work_hardcover_book:
                error_dialog(self, _('Invalid book'), _('No hardcover work id found for this book'), show=True)
                return
            work_id = work_hardcover_book['hardcover_work_id']
        ...
        if d.result() != d.Accepted:
            return
        new_hardcover_book = d.selected_hardcover_book()
        # Replace id and title
        hardcover_book['hardcover_id'] = new_hardcover_book['hardcover_id']
        hardcover_book['hardcover_title'] = new_hardcover_book['hardcover_title']
        row = self.pick_book_table.currentRow()
        self.pick_book_table.populate_table_row(row, hardcover_book)


class PickCalibreBookTableWidget(QTableWidget):

    def __init__(self, parent, hardcover_id):
        QTableWidget.__init__(self, parent)
    ...
    def populate_table_row(self, row, calibre_book):
        existing_hardcover_id = calibre_book.get('hardcover_id', None)
        item = CalibreTitleWidgetItem(calibre_book['calibre_title'],
                                            existing_hardcover_id, self.hardcover_id,
...
class PickCalibreBookDialog(SizePersistedDialog):
    ...
    that can then be linked to a particular hardcover book
    '''
    def __init__(self, parent, id_caches, hardcover_book, calibre_books, search_calibre_fn, next_book):
        SizePersistedDialog.__init__(self, parent, 'hardcover sync plugin:pick calibre book dialog')
        self.id_caches, self.hardcover_book, self.search_calibre_fn = \
            (id_caches, hardcover_book, search_calibre_fn)
        self.skip = False
        ...
        match_groupbox = QGroupBox(_('hardcover book:'))
        layout.addWidget(match_groupbox)
        match_layout = QGridLayout()
        match_groupbox.setLayout(match_layout)
        match_layout.addWidget(QLabel(_('Title:'), self), 0, 0, 1, 1)
        match_layout.addWidget(ReadOnlyLineEdit(hardcover_book['hardcover_title'], self), 0, 1, 1, 3)
        match_layout.addWidget(QLabel(_('Author:'), self), 1, 0, 1, 1)
        match_layout.addWidget(ReadOnlyLineEdit(hardcover_book['hardcover_author'], self), 1, 1, 1, 3)
        match_layout.addWidget(QLabel(_('Series:'), self), 2, 0, 1, 1)
        match_layout.addWidget(ReadOnlyLineEdit(hardcover_book['hardcover_series'], self), 2, 1, 1, 1)
        match_layout.addWidget(QLabel(_('ISBN:'), self), 2, 2, 1, 1)
        match_layout.addWidget(ReadOnlyLineEdit(hardcover_book['hardcover_isbn'], self), 2, 3, 1, 1)

        layout.addSpacing(5)
        layout.addWidget(QLabel(_('Select a calibre book to link to this hardcover book:'), self))
        self.pick_book_table = PickCalibreBookTableWidget(self, hardcover_book['hardcover_id'])
        layout.addWidget(self.pick_book_table)
        self.pick_book_table.doubleClicked.connect(self.accept)
        ...
        words = hardcover_book['hardcover_title'].strip().split()
        ...
        author = hardcover_book['hardcover_author'].replace('.', '. ').replace(',', ' ').replace('  ', ' ')
    ...
    def ok_clicked(self):
        ...
        # is already linked to another hardcover book. We will prompt the user
        # if this is the case giving them the option to overwrite it.
        results = self.selected_results()
        if len(results) == 0:
            self.reject()
            return
        # TODO: Change this when supporting multiple calibre books for a hardcover one
        calibre_id = results[0]['calibre_id']
        hardcover_id = self.id_caches.calibre_to_hardcover_ids().get(calibre_id, '')
        if not hardcover_id or hardcover_id == self.hardcover_book['hardcover_id']:
            self.accept()
            return
        if not question_dialog(self, _('Overwrite hardcover Link'), \
                _('This calibre book is already linked to a different hardcover book.')+'<p>'+ \
                _('Only one hardcover book can be linked to a calibre book at a time.')+'<p><p>'+ \
...
class DoAddRemoveDialog(SizePersistedDialog):
    ...
    def __init__(self, parent, grhttp, id_caches, user_name, action, calibre_books):
        SizePersistedDialog.__init__(self, parent, 'hardcover sync plugin:add remove dialog')
        self.gui = parent
        self.grhttp, self.id_caches, self.user_name, self.action, self.calibre_books = \
            (grhttp, id_caches, user_name, action, calibre_books)
        self.update_isbn = cfg.plugin_prefs[cfg.STORE_PLUGIN].get(cfg.KEY_UPDATE_ISBN, 'NEVER')
    ...
    def init_gui_layout(self):
        self.setWindowTitle('Modify hardcover Shelf')
        layout = QVBoxLayout(self)
        self.setLayout(layout)
        ...
        self.rating_label = QLabel(_('Add calibre Rating to hardcover'), self)
        ...
        self.date_read_label = QLabel(_('Add calibre Date Read to hardcover'), self)
        ...
        self.review_text_label = QLabel(_('Add calibre Review Text to hardcover'), self)
    ...
    def action_button_clicked(self):
        ...
        for calibre_book in self.calibre_books:
            if calibre_book['status'] == ActionStatus.VALID:
                hardcover_id = calibre_book['hardcover_id']
                for shelf_name in selected_shelves:
                    review_id = self.grhttp.add_remove_book_to_shelf(client, shelf_name, hardcover_id, self.action)
                    if not review_id:
                        # Could have had a hardcover failure, stop immediately.
                        break
                # If adding books and rating/date read columns update the hardcover review
                if review_id and self.action == 'add':
                    ...
                    if review_id and (upload_rating or upload_date_read or upload_review_text):
                        ...
                        if upload_review_text:
                            review_text = calibre_book['calibre_review_text']
                            calibre_book['hardcover_review_text'] = review_text
                        self.grhttp.update_review(client, shelf_name, review_id, hardcover_id, rating, date_read, review_text)
                if not review_id:
...
class DoShelfSyncTableWidget(QTableWidget):
    ...
    def populate_table(self, hardcover_books):
        self.clear()
        self.setAlternatingRowColors(True)
        self.setRowCount(len(hardcover_books))
        ...
        self.hardcover_books = hardcover_books
        for row, book in enumerate(self.hardcover_books):
    ...
    def populate_table_row(self, row, hardcover_book, book_index=-1):
        self.blockSignals(True)
        self.setSortingEnabled(False)
        self.setItem(row,  0, SyncStatusDataWidgetItem(hardcover_book['status_msg'], hardcover_book['status']))
        item = ReadOnlyTableWidgetItem(hardcover_book['hardcover_title'])
        if book_index >= 0:
            item.setData(Qt.UserRole, book_index)
        self.setItem(row,  1, item)
        self.setItem(row,  2, ReadOnlyTableWidgetItem(hardcover_book['hardcover_author']))
        self.setItem(row,  3, ReadOnlyTableWidgetItem(hardcover_book['hardcover_series']))
        self.setItem(row,  4, RatingTableWidgetItem(hardcover_book['hardcover_rating'] *2 , is_read_only=True))
        self.setItem(row,  5, DateTableWidgetItem(hardcover_book['hardcover_read_at'], is_read_only=True, fmt=self.format))
        self.setItem(row,  6, ReadOnlyTableWidgetItem(hardcover_book['hardcover_isbn']))
        self.setItem(row,  7, ReadOnlyTableWidgetItem(hardcover_book['hardcover_shelves']))
        self.setItem(row,  8, SortableReadOnlyTableWidgetItem(hardcover_book['calibre_title'], sort_key=hardcover_book['calibre_title_sort']))
        self.setItem(row,  9, SortableReadOnlyTableWidgetItem(hardcover_book['calibre_author'], sort_key=hardcover_book['calibre_author_sort']))
        self.setItem(row, 10, ReadOnlyTableWidgetItem(hardcover_book['calibre_series']))
        self.setItem(row, 11, RatingTableWidgetItem(hardcover_book['calibre_rating'], is_read_only=True))
        self.setItem(row, 12, DateTableWidgetItem(hardcover_book['calibre_date_read'], is_read_only=True, fmt=self.format))
        self.setItem(row, 13, ReadOnlyTableWidgetItem(hardcover_book['calibre_isbn']))
        if book_index >= 0:
    ...
    def item_selection_changed(self):
        ...
        if not self.selectionModel().hasSelection():
            ...
            for row in self.selectionModel().selectedRows():
                book = self.hardcover_books[self.item(row.row(), 1).data(Qt.UserRole)]
                if book['status'] == ActionStatus.WARNING:
                    add_empty_is_valid = False
                if book['status'] == ActionStatus.VALID:
                    add_empty_is_valid = False
                    selection_is_not_valid = False
                if len(book['hardcover_id']) > 0:
                    selection_has_no_hardcover_id = False
        self.search_action.setEnabled(selection_is_not_valid)
        self.book_selection_changed.emit(selection_is_not_valid)
    ...
    def view_book_on_hardcover_click(self):
        for row in self.selectionModel().selectedRows():
            book = self.hardcover_books[self.item(row.row(), 1).data(Qt.UserRole)]
            self.view_book.emit(book['hardcover_id'])
    ...
    def search_for_calibre_books_click(self):
        ...
        if len(rows) == 0:
            return
        self.search_for_hardcover_books.emit(rows, books)
...
class DoShelfSyncDialog(SizePersistedDialog):
    ...
    '''
    def __init__(self, parent, plugin_action, grhttp, user_name, selected_shelves, hardcover_books, calibre_searcher):
        SizePersistedDialog.__init__(self, parent, 'hardcover sync plugin:do shelf sync dialog')
        ...
        self.hardcover_books = self.flatten_book_shelf(hardcover_books)
        for book in self.hardcover_books:
            self.update_book_status(book)
        self.summary_table.populate_table(self.hardcover_books)
        self.update_error_counts()

    def init_gui_layout(self):
        window_text = _('Sync from hardcover Shelf')
        ...
        self.summary_table.view_book.connect(self.grhttp.view_book_on_hardcover)
        self.summary_table.search_for_hardcover_books.connect(self.handle_search_calibre_for_hardcover_books)
        ...
        if self.update_rating:
            ...
            rating_label = QLabel(_('Add hardcover Rating to calibre'), self)
        ...
        if self.update_date_read:
            ...
            date_read_label = QLabel(_('Add hardcover Date Read to calibre'), self)
        ...
        if self.update_review_text:
            ...
            review_text_label = QLabel(_('Add hardcover Review Text to calibre'), self)
    ...
    def flatten_book_shelf(self, hardcover_books):
        ...
        gr_cache = self.calibre_searcher.id_caches.hardcover_to_calibre_ids()
        cb_cache = self.calibre_searcher.id_caches.calibre_to_hardcover_ids()
        # Flatten out the structure, from its 1:M to a 1:1
        flattened_books = []
        autolinked_calibre_ids = []
        for hardcover_id, hardcover_book in hardcover_books.items():
            calibre_ids = gr_cache.get(hardcover_id, [])
            is_auto_link_by_isbn = False
            if len(calibre_ids) == 0 and hardcover_book['hardcover_isbn']:
                # We have not yet mapped this hardcover id to a calibre book
                # Attempt a mapping ourselves by doing a calibre isbn map lookup
                calibre_ids = isbn_map.get(hardcover_book['hardcover_isbn'], [])
                if len(calibre_ids) > 0:
                    # We found at least one calibre book matching the hardcover ISBN
                    # However we can only automap where the calibre book is not mapped
                    # to a different hardcover book already, so filter this list
            ...
            if len(calibre_ids) > 0:
                ...
                for calibre_id in calibre_ids:
                    ...
                    if is_auto_link_by_isbn:
                        ...
                        update_calibre_isbn_if_required(book, book['hardcover_isbn'])
    ...
    def handle_add_empty_books(self, rows, hardcover_books):
        # Treat it as a toggle - if the book has been added already clear the calibre entries
        toggled_ids = []
        for index, row in enumerate(rows):
            hardcover_book = hardcover_books[index]
            if hardcover_book['status'] == ActionStatus.ADD_EMPTY:
                hardcover_book['calibre_id'] = ''
                hardcover_book['calibre_isbn'] = ''
                hardcover_book['calibre_title'] = ''
                hardcover_book['calibre_title_sort'] = ''
                hardcover_book['calibre_author'] = ''
                hardcover_book['calibre_author_sort'] = ''
                hardcover_book['calibre_series'] = ''
                hardcover_book['calibre_rating'] = 0
                hardcover_book['calibre_date_read'] = UNDEFINED_DATE
                hardcover_book['calibre_review_text'] = ''
                self.update_book_status(hardcover_book)
                self.summary_table.populate_table_row(row, hardcover_book, book_index=index)
                toggled_ids.append(hardcover_book['hardcover_id'])

        # Get our setting to know whether to convert hardcover author FN LN to LN, FN
        c = cfg.plugin_prefs[cfg.STORE_PLUGIN]
        swap_author_names = c.get(cfg.KEY_AUTHOR_SWAP, False)
        # Copy the hardcover info into the calibre columns for later usage when we create
        # the actual rows in database
        for index, row in enumerate(rows):
            hardcover_book = hardcover_books[index]
            if hardcover_book['hardcover_id'] in toggled_ids:
                continue
            if hardcover_book['status'] != ActionStatus.NO_LINK:
                continue
            hardcover_book['calibre_isbn'] = hardcover_book['hardcover_isbn']
            hardcover_book['calibre_title'] = hardcover_book['hardcover_title']
            hardcover_book['calibre_author'] = hardcover_book['hardcover_author']
            if swap_author_names:
                authors = hardcover_book['hardcover_author'].split('&')
                swapped_authors = []
                ...
                hardcover_book['calibre_author'] = ' & '.join(swapped_authors)
            hardcover_book['calibre_series'] = hardcover_book['hardcover_series']
            hardcover_book['status'] = ActionStatus.ADD_EMPTY
            hardcover_book['status_msg'] = _('Add to calibre')
            self.summary_table.populate_table_row(row, hardcover_book, book_index=index)
        # Ensure our error counts reflect the latest info
        self.update_error_counts()

    def handle_search_calibre_for_hardcover_books(self, rows, hardcover_books):
        for index, row in enumerate(rows):
            hardcover_book = hardcover_books[index]
            title = hardcover_book['hardcover_title']
            authors = hardcover_book['hardcover_author'].split('&')
            ...
            if self.auto_match_checkbox.isChecked() and len(calibre_books) == 1:
                ...
                # calibre book that is already linked to a different hardcover book
                if not calibre_books[0]['hardcover_id']:
                    self.link_to_calibre_books(row, hardcover_book, calibre_books)
                    continue

            next_book = None
            if index < len(hardcover_books) - 1:
                next_book = hardcover_books[index + 1]['hardcover_title']
            d = PickCalibreBookDialog(self, self.calibre_searcher.id_caches, hardcover_book,
            ...
            if len(selected_books) == 0:
                continue
            self.link_to_calibre_books(row, hardcover_book, selected_books)
        self.update_error_counts()
    ...
    def update_search_results_with_current_links(self, calibre_books):
        # We may need to override the mapped hardcover id on the search results if
        # the user has duplicate hardcover on their shelf and overwrote a link
        # As we have not committed to a database yet the in memory cache is out of date
        for calibre_book in calibre_books:
            calibre_id = calibre_book['calibre_id']
            for hardcover_book in self.hardcover_books:
                is_book_changed = hardcover_book['calibre_id'] != hardcover_book['orig_calibre_id']
                if is_book_changed and calibre_id == hardcover_book['calibre_id']:
                    calibre_book['hardcover_id'] = hardcover_book['calibre_id']
                    break

    def link_to_calibre_books(self, row, hardcover_book, calibre_books):
        ...
        hardcover_book['calibre_id'] = calibre_id
        hardcover_book['calibre_isbn'] = calibre_books[0]['calibre_isbn']
        hardcover_book['calibre_title'] = calibre_books[0]['calibre_title']
        hardcover_book['calibre_author'] = calibre_books[0]['calibre_author']
        hardcover_book['calibre_series'] = calibre_books[0]['calibre_series']
        hardcover_book['calibre_rating'] = calibre_books[0]['calibre_rating']
        hardcover_book['calibre_date_read'] = calibre_books[0]['calibre_date_read']
        hardcover_book['calibre_review_text'] = calibre_books[0]['calibre_review_text']
        update_calibre_isbn_if_required(hardcover_book, hardcover_book['hardcover_isbn'])
        self.update_book_status(hardcover_book)
        self.summary_table.find_and_populate_table_row(row, hardcover_book)
        # We also need to check whether we now have a duplicate link situation
        # where another hardcover book is also linked to this hardcover id.
        # for other_book in self.hardcover_books:
        for index, other_book in enumerate(self.hardcover_books):
            if other_book['hardcover_id'] == hardcover_book['hardcover_id']:
    ...
    def _create_empty_books(self, hardcover_books):
        # Iterate through these hardcover books creating equivalent books in calibre
    ...
    def sync_button_clicked(self):
        ...
        add_empty_books = [b for b in self.hardcover_books if b['status'] == ActionStatus.ADD_EMPTY ]
        ...
        all_sync_books = [b for b in self.hardcover_books if b['status'] in \
                                [ActionStatus.ADD_EMPTY, ActionStatus.VALID] ]
        for shelf in self.shelves:
            sync_actions = []
            # Find the subset of valid books that are on this shelf
            sync_books = [b for b in all_sync_books if shelf['name'] in b['hardcover_shelves_list']]
            if len(sync_books) > 0:
                ...
                if self.update_rating and shelf.get(cfg.KEY_SYNC_RATING):
                    ...
                    for book in sync_books:
                        if book['calibre_rating'] != book['hardcover_rating'] * 2:
                            book['calibre_rating'] = book['hardcover_rating'] * 2
                            book['rating_changed'] = True
                            if not added_action:
                                added_action = True
                                sync_rating_action = {'action':'ADD', 'column':self.rating_column, 'value':'hardcover_rating'}
                                sync_actions.append(sync_rating_action)
                if self.update_date_read and shelf.get(cfg.KEY_SYNC_DATE_READ):
                    added_action = False
                    for book in sync_books:
                        if book['calibre_date_read'] != book['hardcover_read_at']:
                            book['calibre_date_read'] = book['hardcover_read_at']
                ...
                if self.update_review_text and shelf.get(cfg.KEY_SYNC_REVIEW_TEXT):
                    added_action = False
                    for book in sync_books:
                        if book['calibre_review_text'] != book['hardcover_review_text']:
                            book['calibre_review_text'] = book['hardcover_review_text']
