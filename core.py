...
import calibre_plugins.hardcover_sync.oauth2 as oauth
import calibre_plugins.hardcover_sync.httplib2 as httplib2
...
def get_searchable_author(authors):
    ...
    # pass to the hardcover website in FN LN format for just the first author.
    # We do this because hardcover uses FN LN format and can get grumpy when it isn't.
...
def update_calibre_isbn_if_required(calibre_book, hardcover_isbn, update_isbn=None):
    if not update_isbn:
    ...
    if update_isbn == 'ALWAYS':
        calibre_book['calibre_isbn'] = hardcover_isbn
        return
    # else only overwrite if missing
    if hardcover_isbn and not calibre_book['calibre_isbn']:
        calibre_book['calibre_isbn'] = hardcover_isbn
...
class HttpHelper(object):
    ...
    def _oauth_request_get(self, oauth_client, url, success_status='200'):
        ...
        try:
            if self.gui:
                self.gui.status_bar.showMessage('Communicating with hardcover...')
    ...
    def _handle_failure(self, response, content, url):
        if DEBUG:
            debug_print('hardcover failure calling: %s' % url)
            debug_print('Response: %s' % response)
            debug_print('Content: %s' % content)
            traceback.print_stack()
        detail = 'URL: {0}\nResponse Code: {1}\n{2}'.format(url, response['status'], content)
        error_dialog(self.gui, _('hardcover Failure'),
                     _('The request contacting hardcover has failed.')+'\n'+
                     _('If it reoccurs you may have exceeded a request limit imposed by hardcover.')+'\n'+
    ...
    def view_book_on_hardcover(self, hardcover_id):
        url = '%s/book/show/%s' % (cfg.URL_HTTPS, hardcover_id)
        open_url(QUrl(url))
    ...
    def get_hardcover_user_id(self, oauth_token, oauth_secret):
        # Returns the hardcover user id for this token/secret, None if an error
        oauth_client = self.create_oauth_client(oauth_token=oauth_token, oauth_secret=oauth_secret)
...
#             debug_print("get_shelf_list: content=", content)
            # Get the latest list of shelf names, order same as they are set on hardcover
            root = et.fromstring(content)
    ...
    def add_remove_book_to_shelf(self, oauth_client, shelf_name, hardcover_id, action='add'):
        # Return Review id if book was added.
        url = '%s/shelf/add_to_shelf.xml' % cfg.URL_HTTPS
        body_info = {'name': shelf_name, 'book_id': hardcover_id}
        if DEBUG:
        ...
        if _response:
            if action == 'add':
                ...
                # Don't currently have the actual response hardcover send to detect this.
    ...
    def create_review(self, oauth_client, shelf_name, hardcover_id, rating, date_read, review_text):
        # Return True if review was created, False if not. Not currently used in code
        url = '%s/review.xml' % (cfg.URL_HTTPS, )
        body_info = { 'book_id': hardcover_id }
    ...
    def get_hardcover_id_for_isbn(self, isbn):
        # Returns a hardcover_id for a given ISBN from hardcover, None if an error or not found
    ...
    def get_hardcover_books_on_shelves(self, user_name, shelves, per_page=100):
        # Returns a dictionary of books on these hardcover shelf by hardcover id, None if an error
        debug_print("HttpHelper::get_hardcover_books_on_shelves: user_name=%s" % (user_name, ))
        oauth_client = self.create_oauth_client(user_name)
        ...
        for shelf in shelves:
            ...
            while True:
                ...
                # Will need to retrieve books in pages with multiple calls if many on the list
                page = page + 1
                debug_print("HttpHelper::get_hardcover_books_on_shelves: shelf='%s', page=%s" % (shelf_name, page))
...
#                 debug_print("get_hardcover_books_on_shelves: content=", content)
#                 open('E:\\test.xml','w').write(content)
                ...
                for review_node in review_nodes:
                    ...
                    if book and book['hardcover_id'] not in shelf_books:
                        shelf_books[book['hardcover_id']] = book
                if end >= total:
                    break
        return shelf_books

    def get_review_book(self, user_name, hardcover_id):
        # Get the user's review for a book
        users = cfg.plugin_prefs[cfg.STORE_USERS]
        user_id = users[user_name][cfg.KEY_USER_ID]
        url = '%s/review/show_by_user_and_book.xml?user_id=%s&book_id=%s' % \
            (cfg.URL_HTTPS, user_id, hardcover_id)
        (response, content) = self._request_get(url, suppress_status='404')
        ...
        if response['status'] == '404':
            if DEBUG:
                debug_print('User \'%s\' does not have a review for book: %s' %(user_name, hardcover_id))
            return
...
#         debug_print('get_review_book: show content=%s' %(content,))
    ...
    def search_for_hardcover_books(self, title='', authors=''):
        ...
        for work_node in work_nodes:
            book = {}
            book['hardcover_work_id'] = work_node.findtext('id')
            book['hardcover_id'] = work_node.findtext('best_book/id')
            book['hardcover_author'] = work_node.findtext('best_book/author/name')
            if book['hardcover_author'] == 'NOT A BOOK':
                # hardcover use this author to categorise ISBNs in their databases that
                # are not actually books
                continue
            (title, series) = self._convert_hardcover_title_with_series(work_node.findtext('best_book/title').strip())
            book['hardcover_title'] = title
            book['hardcover_series'] = series
            search_books.append(book)
        return search_books

    def get_hardcover_book_for_id(self, hardcover_id):
        ...
        url = '%s/book/show?format=json&id=%s&page=1' % (cfg.URL_HTTPS, hardcover_id)
        (response, content) = self._request_get(url)
        if not response:
            return
    ...
    def get_hardcover_book_with_work_id(self, hardcover_id):
        # Returns hardcover book with extra information related to the work id, which is used
        # so we can then use to query for other editions. Not available in the json query above, grrr.
        url = '%s/book/show?format=xml&id=%s&page=1' % (cfg.URL_HTTPS, hardcover_id)
        (response, content) = self._request_get(url)
        if not response:
            return
        content = clean_ascii_chars(content)
    ...
    def convert_json_to_book(self, content_json):
        book = {}
        book['hardcover_id'] = str(content_json['id'])
        book['hardcover_isbn'] = content_json.get('isbn13', '')
        if not book['hardcover_isbn']:
            book['hardcover_isbn'] = content_json.get('isbn', '')
        ...
        book['hardcover_author'] = authors_text
        (title, series) = self._convert_hardcover_title_with_series(content_json['title'].strip())
        book['hardcover_title'] = title
        book['hardcover_series'] = series
        # Stuff that is not available at JSON level (only at review level)
        book['hardcover_shelves'] = ''
        book['hardcover_shelves_list'] = []
        book['hardcover_review_id'] = ''
        book['hardcover_rating'] = 0
        book['hardcover_started_at'] = UNDEFINED_DATE
        book['hardcover_read_at'] = UNDEFINED_DATE
        book['hardcover_date_added'] = UNDEFINED_DATE
        book['hardcover_date_updated'] = UNDEFINED_DATE
        book['hardcover_review_text'] = ''
        return book
...
#         debug_print("HttpHelper::_convert_review_xml_node_to_book - review_node=", tostring(review_node))
        ...
        book = {}
        hardcover_id = book_node.findtext('id')
        book['hardcover_id'] = hardcover_id
        isbn = book_node.findtext('isbn13')
        book['hardcover_isbn'] = isbn
        (title, series) = self._convert_hardcover_title_with_series(book_node.findtext('title').strip())
        book['hardcover_title'] = title
        book['hardcover_series'] = series
        if book_node.find('authors') is None:
            # We have an error situation where the returned xml is being corrupted due to the
            # hardcover bug for encodings. We will skip this book
            if DEBUG:
                debug_print('hardcover shelf error due to corruption bug. Skipping book: %s' % book)
            return None
        author_nodes = book_node.findall('authors/author')
        authors = [author_node.findtext('name').strip() for author_node in author_nodes]
        book['hardcover_author'] = '& '.join(authors)
        ...
        if shelve_nodes is not None and len(shelve_nodes):
            ...
            book['hardcover_shelves'] = ', '.join(shelves)
            book['hardcover_shelves_list'] = shelves
            book['hardcover_review_id'] = review_node.findtext('id')
            book['hardcover_rating'] = int(review_node.findtext('rating'))
            # Get the various date fields in case the user has sync actions for them
            book['hardcover_started_at'] = self._parse_hardcover_date(review_node.findtext('started_at'))
            book['hardcover_read_at'] = self._parse_hardcover_date(review_node.findtext('read_at'))
            book['hardcover_date_added'] = self._parse_hardcover_date(review_node.findtext('date_added'))
            book['hardcover_date_updated'] = self._parse_hardcover_date(review_node.findtext('date_updated'))
            #review_text = review_node.findtext('body')
            book['hardcover_review_text'] = review_node.findtext('body').strip()
            if len(book['hardcover_review_text']) > 0:
                debug_print("_convert_review_xml_node_to_book: length of review_text=", len(book['hardcover_review_text']))
#                 debug_print("_convert_review_xml_node_to_book: review_text=", book['hardcover_review_text'])
        else:
            book['hardcover_shelves'] = ''
            book['hardcover_shelves_list'] = []
            book['hardcover_review_id'] = ''
            book['hardcover_rating'] = 0
            book['hardcover_started_at'] = ''
            book['hardcover_read_at'] = ''
            book['hardcover_date_added'] = ''
            book['hardcover_date_updated'] = ''
            book['hardcover_review_text'] = ''
        if include_work:
    ...
    def _parse_hardcover_date(self, date_text):
        if date_text == '':
            return UNDEFINED_DATE
        return parse_date(date_text, assume_utc=True, as_utc=False)

    def _convert_hardcover_title_with_series(self, text):
        # This function attempts to convert a myriad of hardcover title
    ...
    def get_xml_tree(self, content):
        ...
        if root is None:
            ...
            f.write(content)
            f.close()
            raise ValueError('The shelf contains a corrupting response from hardcover. ' +
    ...
    def get_edition_books_for_work_id(self, work_id):
        ...
        url = 'https://www.hardcover.com/work/editions/%s'%work_id
        try:
            br = browser()
            ...
            if error_code == 404:
                ...
                msg = 'Failed to query hardcover: %r'%url
            if error_code:
                msg = msg + '\nError Code: %s' % error_code
            if getattr(e, 'reason', None):
                msg = msg + '\nError Reason: %s' % e.reason
            error_dialog(self.gui, 'hardcover Failure',
                         'The request contacting hardcover has failed. Please try again.',
                         det_msg=msg, show=True)
            return None
        raw = raw.decode('utf-8', errors='replace')
        root = fromstring(clean_ascii_chars(raw))

        hardcover_edition_books = []
        ...
        for edition_node in edition_nodes:
            ...
            for idx, data_row_node in enumerate(edition_data_node.xpath('div[@class="dataRow"]')):
                ...
                if text.startswith('Published'):
                    continue
                hardcover_edition_book['hardcover_edition'] = text
                break

            book_url = ''.join(edition_data_node.xpath('div[@class="dataRow"]/a/@href'))
            hardcover_edition_book['hardcover_id'] = re.search('/book/show/(\d+)', book_url).groups(0)[0]
            hardcover_edition_book['hardcover_title'] = ''.join(edition_data_node.xpath('div[@class="dataRow"]/a[@class="bookTitle"]/text()'))
            cover_url = ''.join(edition_node.xpath('div[@class="leftAlignedImage"]/a/img/@src'))
            if 'nocover' in cover_url:
                hardcover_edition_book['hardcover_cover'] = 'No'
            else:
                hardcover_edition_book['hardcover_cover'] = 'Yes'
            hardcover_edition_book['hardcover_isbn'] = ''
            ...
            if len(isbn_node) > 0:
                ...
                if match_isbn:
                    ...
                    if check_isbn(isbn):
                        hardcover_edition_book['hardcover_isbn'] = isbn
            hardcover_edition_books.append(hardcover_edition_book)

        return hardcover_edition_books
...
class IdCaches(object):
    ...
    def invalidate_caches(self):
        self._hardcover_to_calibre_id_cache = None
        self._calibre_to_hardcover_id_cache = None

    def hardcover_to_calibre_ids(self):
        # Construct on demand a cache mapping hardcover ids to Calibre ids.
        if self._hardcover_to_calibre_id_cache is None:
            self.build_caches()
        return self._hardcover_to_calibre_id_cache

    def calibre_to_hardcover_ids(self):
        # Construct on demand a cache mapping Calibre ids to hardcover ids.
        # This is purely a performance optimisation.
        if self._calibre_to_hardcover_id_cache is None:
            self.build_caches()
        return self._calibre_to_hardcover_id_cache

    def get_calibre_ids_linked(self, hardcover_id):
        # Offers a safe way of retrieving calibre ids linked to this hardcover id
        # by checking to see if each calibre id has been deleted first.
        existing_calibre_ids = self.hardcover_to_calibre_ids().get(hardcover_id, [])
        valid_ids = []
    ...
    def build_caches(self):
        ...
        cb_cache = {}
        db = self.gui.library_view.model().db
        calibre_ids = db.data.search_getting_ids('identifiers:hardcover:True', search_restriction='')
        for calibre_id in calibre_ids:
            hardcover_id = db.get_identifiers(calibre_id, index_is_id=True).get('hardcover', '')
            if hardcover_id:
                # We will allow multiple Calibre ids per hardcover id.
                calibre_ids_mapped = gr_cache.get(hardcover_id, [])
                calibre_ids_mapped.append(calibre_id)
                gr_cache[hardcover_id] = calibre_ids_mapped
                cb_cache[calibre_id] = hardcover_id
        self._hardcover_to_calibre_id_cache = gr_cache
        self._calibre_to_hardcover_id_cache = cb_cache

    def remove_calibre_id_from_cache(self, deleted_id):
        if not self._calibre_to_hardcover_id_cache:
            return
        if deleted_id in self._calibre_to_hardcover_id_cache:
            hardcover_id = self._calibre_to_hardcover_id_cache[deleted_id]
            del self._calibre_to_hardcover_id_cache[deleted_id]
            calibre_ids_mapped = self._hardcover_to_calibre_id_cache[hardcover_id]
            calibre_ids_mapped.remove(deleted_id)
            if len(calibre_ids_mapped) == 0:
                del self._hardcover_to_calibre_id_cache[hardcover_id]
            else:
                self._hardcover_to_calibre_id_cache[hardcover_id] = calibre_ids_mapped
...
class CalibreDbHelper(object):
    ...
    def apply_actions_to_calibre(self, gui, hardcover_books, actions):
        self.gui = gui
        ...
        for sync_action in actions:
            if sync_action['column'] == 'tags':
                self._apply_tag_changes_to_books(hardcover_books, sync_action['action'], sync_action['value'])
            elif sync_action['column'] == 'rating':
                self._apply_rating_changes_to_books(hardcover_books, sync_action['action'], sync_action['value'])
            elif sync_action['column'] == 'comments':
                self._apply_comment_changes_to_books(hardcover_books, sync_action['action'], sync_action['value'])
            else:
                # Applying values to a custom column
                self._apply_custom_column_changes_to_books(hardcover_books, sync_action['column'],
                                                          sync_action['action'], sync_action['value'],
                                                          sync_action.get('special',''))
        self.db.commit()

    def _apply_tag_changes_to_books(self, hardcover_books, action, value):
        '''
        Apply changes to a tags column.
        '''
        ids = [book['calibre_id'] for book in hardcover_books]
        value = value.split(',')
        if action == 'ADD':
    ...
    def _apply_rating_changes_to_books(self, hardcover_books, action, value):
        '''
        ...
        for book in hardcover_books:
            ...
            if action == 'ADD':
                if value == 'none':
                    ...
                    rating_value = book.get('hardcover_rating','0')
                    # Adapt the hardcover value (0-5) to calibre db value (range 0-10)
    ...
    def _apply_comment_changes_to_books(self, hardcover_books, action, value):
        '''
        Apply changes to a rating column. Currently there is only one way this is allowed which is
        ...
        for book in hardcover_books:
            ...
            if action == 'ADD':
                if value == 'none':
                    ...
                    new_value = book['hardcover_review_text']
            elif action == 'REMOVE':
                new_value = '' # Value for any REMOVE action
            if new_value != existing_value:
                self.db.set_comment(calibre_id, new_value, notify=False, commit=False)

    def _apply_custom_column_changes_to_books(self, hardcover_books, column, action, value, special_action):
        '''
        ...
        if typ == 'bool':
            ...
            for book in hardcover_books:
                ...
                if action == 'ADD':
                    if value == 'none':
                        ...
                        try:
                            date_value = book['hardcover_'+value]
                            if date_value:
            ...
            for book in hardcover_books:
                ...
                if special_action == 'review_text':
                    value = book['hardcover_review_text']
                    if typ != 'comments':
